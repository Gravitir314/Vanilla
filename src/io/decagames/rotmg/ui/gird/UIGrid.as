...
package io.decagames.rotmg.ui.gird
{
    ...
    public class UIGrid extends Sprite 
    {
        ...
        private var lastRenderedItemsNumber:* = 0;

        public function UIGrid(_arg_1:int, _arg_2:int, _arg_3:int, _arg_4:int=-1)
        {
        ...
        public function get numberOfElement():int
        {
        ...
        public function addDecoration(_arg_1:String):void
        {
            var _local_6:int;
            var _local_7:SliceScalingBitmap;
            var _local_2:int = int(((this.gridWidth - ((this.numberOfColumns - 1) * this.gridMargin)) / this.numberOfColumns));
            var _local_3:int = (this.numberOfColumns - 1);
            var _local_4:int = int(Math.ceil((this.elements.length / this.numberOfColumns)));
            if (_local_4 == 0)
            {
                _local_4 = 1;
            };
            ...
            while (_local_5 < _local_4)
            {
                ...
                {
                    _local_7 = TextureParser.instance.getSliceScalingBitmap("UI", _arg_1);
                    _local_7.x = (((_local_6 * (this.gridMargin / 2)) + ((_local_6 + 1) * (_local_2 + (this.gridMargin / 2)))) - (_local_7.width / 2));
                    _local_7.y = (189 - (_local_7.height / 2));
                    this.gridContent.addChild(_local_7);
                    this.decors.push(_local_7);
                    _local_6++;
                };
                _local_5++;
            };
        ...
        public function render():void
        {
            ...
            var _local_1:int = int(((this.gridWidth - ((this.numberOfColumns - 1) * this.gridMargin)) / this.numberOfColumns));
            var _local_2:int = 1;
            var _local_3:int;
            var _local_4:int;
            var _local_5:int;
            var _local_6:int = int(Math.ceil((this.elements.length / this.numberOfColumns)));
            var _local_7:int = 1;
            for each (_local_8 in this.elements)
            {
                _local_8.resize(_local_1);
                if (_local_8.height > _local_5)
                {
                    _local_5 = _local_8.height;
                };
                _local_8.x = _local_3;
                _local_8.y = _local_4;
                if (++_local_2 > this.numberOfColumns)
                {
                    _local_7++;
                    _local_3 = 0;
                    if (((_local_7 == _local_6) && (this._centerLastRow)))
                    {
                        _local_9 = ((_local_7 * this.numberOfColumns) - this.elements.length);
                        _local_3 = int(Math.round((((_local_9 * _local_1) + ((_local_9 - 1) * this.gridMargin)) / 2)));
                    };
                    _local_4 = (_local_4 + (_local_5 + this.gridMargin));
                    _local_5 = 0;
                    _local_2 = 1;
                }
                else
                {
                    _local_3 = (_local_3 + (_local_1 + this.gridMargin));
                };
            };
            if (this.scrollHeight != -1)
            {
                ...
                this.gridMask.graphics.drawRect(0, 0, _local_1, this.scrollHeight);
